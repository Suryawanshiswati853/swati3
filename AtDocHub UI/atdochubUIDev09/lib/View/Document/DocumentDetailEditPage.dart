...
import '../AlwaysDisableFocusNode.dart';
...
class DocumentDetialEditPage extends StatefulWidget {
  ...
  String playerid;
  DocumentDetialEditPage(this.docId, this.playerid,this.userName, this.createdBY);
  @override
  _DocumentDetialEditPageState createState() =>
      _DocumentDetialEditPageState(this.docId,this.userName, this.createdBY, this.playerid);
}
...
class _DocumentDetialEditPageState extends State<DocumentDetialEditPage> {
 ...
 final String userName;
  ...
  _DocumentDetialEditPageState(this.docId, this.playerid, this.userName, this.createdBY);
  @override
  void initState() {
    @override
    void initState() {
        ...
        leading: IconButton(
            ...
            onPressed: () => Navigator.of(context).push(MaterialPageRoute(
                builder: (BuildContext context) =>
                    DocumentsDetails(docId, playerid,this.userName)))),
      ),
          ...
          child: FutureBuilder<Document>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                if (snapshot.hasData) {
                  this.title = snapshot.data!.docTitle;
                  return Center(
                    child: SelectionArea(
                      child: Form(
                        key: _formKey,
                                      ...
                                      if (isValidForm) {
                                        ...
                                        Navigator.of(context).push(
                                            MaterialPageRoute(
                                                builder:
                                                    (BuildContext context) =>
                                                        DocumentsDetails(docId,playerid,
                                                            this.userName)));

                                        Future<void> initOneSignal() async {
                                          OneSignal.shared.setAppId(
                                              'e89acaa4-5388-4e3a-bd69-44d197bdcbd7');
                                          final status = await OneSignal.shared
                                              .getDeviceState();
                                          final String? osUserID =
                                              status?.userId;
                                          ...
                                          OneSignal.shared
                                              .setNotificationOpenedHandler(
                                                  (OSNotificationOpenedResult
                                                      result) {
                                            debugPrint(
                                                'NOTIFICATION OPENED HANDLER CALLED WITH: $result');
                                            setState(() {
                                              Navigator.of(context).push(
                                                  MaterialPageRoute(
