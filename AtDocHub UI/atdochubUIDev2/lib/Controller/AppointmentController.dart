...
class AppointmentController {
  ...
  static Future<List<Appointment>> getAptList(String query) async {
    ...
    if (response.statusCode == 200) {
      final List apmnt = json.decode(response.body);
      // return json.decode(response.body).toList();

      return apmnt.map((json) => Appointment.fromJson(json)).where((apt) {
        final titleLower = apt.docTitle.toLowerCase();
        final tokenLower = apt.tokenNo.toString();
