...
class LoginController {
  static Future<List<UserLogin>> loginAdmin() async {
    final response = await http.get(Uri.parse(
        'http://13.233.206.251:8088/AtdochubJ-3/user/login?useremail=reshma@gmail.com&userpassword=1221'));
    // print('email+++ ${email}');
  ...
  static Future<List<UserLogin>> users() async {
    final response = await http
        .get(Uri.parse('http://13.233.206.251:8088/AtdochubJ-3/user/'));
    // print('email+++ ${email}');
    // print('password ${password}');
    //print(fromJson(json)).toList());
    if (response.statusCode == 200) {
  ...
  static Future<List<UserLogin>> login(String email, String password) async {
    ...
    // try {
    final response = await http.get(Uri.parse(
        'http://13.233.206.251:8088/AtdochubJ-3/user/login?useremail=${email}&userpassword=${password}'));
    // print('email+++ ${email}');
    // print('password ${password}');
    //print(fromJson(json)).toList());
