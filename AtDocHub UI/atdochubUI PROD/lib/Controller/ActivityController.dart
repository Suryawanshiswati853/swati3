...
class ActivityController {
  Future<Activity> fetchActivityById(int docId) async {
    final response = await http.get(
      Uri.parse(
          'http://13.233.206.251:8088/AtdochubJ-3/activity/updatebyid/${docId}'),
    );
  ...
  static Future<List<ActivityCount>> userDashboard() async {
    final url = Uri.parse(
        'http://13.233.206.251:8088/AtdochubJ-3/activity/userdashboard/');
  ...
  static Future<List<Activity>> fetchActivity(docId) async {
    ...
    final url =
        Uri.parse('http://13.233.206.251:8088/AtdochubJ-3/activity/${docId}');
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final parsed = json.decode(response.body).cast<Map<String, dynamic>>();
...
Future<Activity> updateActivity(
  ...
  final http.Response response = await http.patch(
    ...
    Uri.parse('http://13.233.206.251:8088/AtdochubJ-3/activity/updatebyid/'),
    headers: {
